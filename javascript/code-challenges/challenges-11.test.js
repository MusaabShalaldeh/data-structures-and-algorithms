...
const divisibleByFiveTwoToThePower = (input) => {
  // remove elements that are not numbers or are not divisible by 5
  let arr1 = input.map((item) => {
    return item.filter((num) => num % 5 === 0 && typeof num === "number");
  });
...
let findShortest = (data) => {
  let lastNum = 0;
  let indexOfLowestCharacter = 0;
  let arrOfHeights = data.map((item,index)=>{
    return [item.height,index];
  })
  for (let i = 0; i < arrOfHeights.length; i++) {
    if(lastNum == 0)
      lastNum = arrOfHeights[i][0];
    if(arrOfHeights[i][0] >= lastNum)
    {
      lastNum = arrOfHeights[i][0];
      indexOfLowestCharacter = arrOfHeights[i][1];
    }
  }

  return data[indexOfLowestCharacter].name;

  //couldn't figure out anoter solution...
};
