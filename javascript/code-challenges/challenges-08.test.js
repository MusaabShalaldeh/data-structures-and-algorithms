...
const isNum = (input) => {
  const regex4 = /[0-9]+/;
  if(input.toString().match(regex4))
  return true;
  else
  return false;
};
...
const containsWorld = (input) => {
  const regex4 = /(?:^|\W)world(?:$|\W)/;
  if(input.match(regex4))
  return true;
  else
  return false;
};
...
const isCapitalized = (str) => {
  ...
  for (let i = 0; i < splitString.length; i++) {
    if(splitString[i].match(regex4))
    {
      newArr.push(splitString[i].replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,""));
    }
  }
...
const citiesAtoJ = (arr) => {
  const regex4 = /\<b[A-J]*ch[A-J]*/g;
  let newArr = arr.map(item=>{
    if(item.match(regex4)) return item;
  })
  return newArr;
};
