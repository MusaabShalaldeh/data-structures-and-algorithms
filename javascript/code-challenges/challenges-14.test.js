...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  const secureStr = 'https://';
  if(url.includes(secureStr))
    return true
  else return false
};
...
const detectTicTacToeWin = (board) => {
  if(checkHorizionatlly(board)||checkVertically(board)||checkDiagnoally(board))
    return true;
  else return false;

  //my messiest code ever... XD atleast it works.
};
function checkHorizionatlly(arr){
  let check = false;
  for (let i = 0; i < arr.length; i++) {
    let row = arr[i].join('');
    if(row ==='XXX' || row ==='OOO')
